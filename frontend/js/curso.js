...
(function () {
  ...
  function renderPurchaseCard(course) {
    ...
    // Includes (based on modules/lessons)
    const includesList = document.getElementById('purchase-includes-list');
    const includes = [
      { icon: 'video', text: course.duration ? `${course.duration} de video bajo demanda` : 'Video bajo demanda' },
      { icon: 'file', text: 'Documentos descargables' },
      { icon: 'check', text: 'Ejercicios y casos prácticos' },
      { icon: 'monitor', text: 'Acceso desde cualquier dispositivo' },
      { icon: 'message', text: 'Soporte de consultas por 30 días' },
    ];
    ...
    includesList.innerHTML = includes.map((inc) =>
      `<div class="include-item">${iconSvg[inc.icon]}<span>${escHtml(inc.text)}</span></div>`
    ).join('');
  }
  ...
  function renderAboutTab(course) {
    ...
    // Audience section — hide if no data
